...
/// Durable local store. Reports survive process termination and connectivity loss.
class OfflineReportStore {
  Database? _database;

  Future<Database> get database async {
    if (_database != null) return _database!;
    final dbPath = path.join(await getDatabasesPath(), 'terraguard_offline.db');
  ...
  Future<void> save(OfflineHazardReport report) async {
    final db = await database;
    await db.insert('hazard_reports', report.toMap(), conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<void> update(OfflineHazardReport report) async {
    await (await database).update('hazard_reports', report.toMap(), where: 'report_id = ?', whereArgs: [report.reportId]);
  }
